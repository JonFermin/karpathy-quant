...
def generate_weights(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Classic 6-1 momentum: 126d return, skipping the most recent 21d.
    mom = prices.pct_change(126).shift(21)

    # Risk-adjust: divide 6-1 mom by 126d daily-return vol.
    rets = prices.pct_change()
    vol = rets.rolling(126).std().shift(21)
    score = mom / vol

    # Top-decile select; skew > -0.5 (crash-risk); vol_rank > 0.1 (data-quality).
    ...
    # Normalize each row to gross leverage 1.0 (or 0 if nothing qualifies).
